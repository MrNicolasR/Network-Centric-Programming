...
// Mutex for the logging part
pthread_mutex_t threadMu;
...
// Create struct that will be passed to thread
struct threadStruct
{
    struct sockaddr_in clientaddr;
    int connfd;
};
typedef struct threadStruct socketData;

/* Proxy Function (Needed to be pointer to pass into thread)
*  Program runs in a thread and gets the client's info / the page info
*  The program then passes the info to be logged/foward respectfully
*  The program ensures the concurrent threads can write to the same logfile  
*/
void *proxy(socketData *params)
{   
    // Variables
    ...
    socketData *argu = (socketData *)params; // Arguements 
    ...
    {
        response_len += n;
        Rio_writen_w(argu->connfd, buf, n);
#if defined(DEBUG)
    ...
    // Mutex lock
    pthread_mutex_lock(&threadMu);
    format_log_entry(log_entry, &argu->clientaddr, request_uri, response_len);
    fprintf(log_file, "%s %d\n", log_entry, response_len);
    fflush(log_file);
    //Mutex unlock
    pthread_mutex_unlock(&threadMu);
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        // Initialize the struct that will be
        // passed into the thread
        socketData *argu = (socketData *)malloc(sizeof(argu)); // Create 
        error = 0; //Used to fix a bug
        clientlen = sizeof(clientaddr); //Client address Length
        connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen); // Connect to server

        // Pass Arguements into the struct
        argu->clientaddr = clientaddr; // Pass client address
        argu->connfd = connfd; // pass the socket 
        // Once connected a thread is created
        // And the proxy function will start 
        pthread_create(&tid, 0, proxy, argu);
        // Once the function is completed detach the thread
        pthread_detach(tid);
    }
 ...
 * 
 * The paramss are the socket address of the requesting client
