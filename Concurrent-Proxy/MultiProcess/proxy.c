...
/* Proxy Function
*  Program runs in a process and gets the client's info / the page info
*  The program then passes the info to be logged/foward respectfully
*  The program ensures the concurrent process can write to the same logfile  
*/
int proxy(int connfd, struct sockaddr_in clientaddr)
{
    // Variables
...
int main(int argc, char **argv)
{
    ...
    fileSema = mmap(NULL, sizeof(sem_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    sem_init(fileSema, 1, 1);
    ...
    while (1)
    {
        ...
        if ((pid = fork()) == 0)
        {
            ...
            proxy(connfd, clientaddr);   // child handles request
        }
